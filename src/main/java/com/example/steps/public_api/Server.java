...
public class Server extends AbstractVerticle {
  ...
  public Completable rxStart() {
    ...
    router.post(prefix + "/register").handler(this::register); // register new user
    router.post(prefix + "/token").handler(this::token); // generate token

    router.get(prefix + "/:username/:year/:month")
      .handler(jwtHandler) // ctx.user()....
      .handler(this::checkUser) // check user
  ...
  private void fetchUser(RoutingContext ctx) {
    webClient
      ...
      .as(BodyCodec.jsonObject()) // convert response to JsonObject
      .rxSend()
      .subscribe(resp -> forwardJsonOrStatusCode(ctx, resp), err -> sendBadGateway(ctx, err));
  }
  ...
  private void forwardJsonOrStatusCode(RoutingContext ctx, HttpResponse<JsonObject> resp) {
    if (resp.statusCode() != 200) {
      ...
      ctx.response().putHeader("Content-Type", "application/json").end(resp.body().encode());
    }
  }
}
